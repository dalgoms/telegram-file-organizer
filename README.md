...
### 실행 전 보호
- 정리 전 항상 미리보기를 보여줌 — 확인 없이는 절대 실행 안 됨
- `C:\Windows`, `C:\Program Files` 등 시스템 폴더 자동 차단
- GPT가 반환한 폴더명에서 `../` 같은 경로 탈출 패턴 자동 제거
- 스캔 후 5분 경과 시 자동 만료 — 변경된 파일로 정리되는 것 방지
- 이전 스캔 버튼 클릭 시 무효 처리 — 최신 결과만 실행 가능

### 실행 중 보호
- 매 파일 이동마다 `history.json`에 즉시 저장 — 크래시 시에도 undo 가능
- 원자적 JSON 쓰기 (`os.replace`) — 파일 저장 중 정전이 나도 이력 보존
- 심볼릭 링크 자동 건너뜀 — 의도치 않은 외부 폴더 파일 이동 방지
- 사용 중인 파일(잠금) 감지 — "다른 프로그램이 열고 있음" 안내
- Windows 260자 경로 제한 감지
- 동일 파일명 충돌 시 자동으로 번호 부여 (`파일_1.ext`)

### 실행 후 보호
- `/undo`로 마지막 정리를 언제든 되돌릴 수 있음
- undo 시 봇이 만든 폴더만 삭제 — 원래 있던 빈 폴더는 보존
